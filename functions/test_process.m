...
%This function allows to detect, depending on detection_mode value,spindles and/or Kcomplex based on %
%
%**********************************************************
%Inputs:
%**********************************************************
...
%5.subj_name:name for EEG data, this name will be used to generate results in .txt files  
%6.epoch_length: length of data winodw in secons (e.g: 30 sec)
%7. varargin depending on detection mode varargin must be : 
...
%   score_auto_subjname_kcomplex.txt: a txt file containing automatic detection results for Kcomplex, the file contain three columns: 
                                                              %1. segment number,
                                                              %2. number of kcomplex detected in this segment,
                                                              %3. the date of event occurence in the segment (sec)
%   score_auto_subjname_spindles.txt a txt file containing automatic detection results for spindes, the file contain four columns:
                                                              %1. segment number,
                                                              %2. number of spindles detected in this segment,
                                                              %3. spindle start time 
                                                              %4. spndles stop time

                                                              
%Example: [nbr_sp,pos_sp,nbr_kc,pos_kc]=test_process(data,fs,subj_name,detection_mode,op_thr_sp,op_thr_kp);
%--------------------------------------------------------------------------------------------------------------------------------------
display('Test Process');
display('Signal decomposition....')

%data=data_epoching(sig,fs,epoch_length);
for k=1:length(data)
